...
public class Controller {
		...
		for(int i = 0 ; i < buttons.size(); ++i) {
			...
			b.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					if(box_src.getValue() != null && box_src.getValue().equals(b.getText()))
						box_src.setValue(null);
					else if(box_src.getValue() != null)
						box_target.setValue(b.getText());
					else
						box_src.setValue(b.getText());
				}
			});
		}
	}
	public void addButton(String name, double x, double y) {
		x = getX(x);
	...
	public void readFile() {
		try {
			...
			int viricies = Integer.parseInt(reader.split(" ")[0]);
			int edges = Integer.parseInt(reader.split(" ")[1]);
			
			for(int i = 0 ; i < viricies; ++i) {
				...
				addButton(tokens[0], Double.parseDouble(tokens[1]), Double.parseDouble(tokens[2]));
			}
			...
			for(int i = 0 ; i < edges ; ++i) {
				...
				for(City key: cities.map.keySet())
	...
	public LinkedList<City> djikarta(City source , City destination , HashMap<City , List<City>> map) {
		LinkedList<City> done = new LinkedList<>() ;
		//done.add(source) ;	
		LinkedList<City> edges = new LinkedList<>();
		LinkedList<City> tempList = new LinkedList<>();
		for(int i = 0 ; i < ((LinkedList<City>) map.get(source)).size() ; ++i) {
			edges.add(new City(((LinkedList<City>) map.get(source)).get(i)));
			tempList.add(new City(((LinkedList<City>) map.get(source)).get(i)));
			}
		...
		while(edges.size() > 0) { // here
			...
			for(int j = 0 ; j < list.size(); ++j) {
				if(!done.contains(list.get(j)) && !list.get(j).equals(source)) { // The Node Is Not Expaned Yet
					if(edges.contains(list.get(j))) {	 // THE NODE IS CONNECTED TO THE SOURCE
														 // DIRECTLY OR BY OTHER NODE
						double cost = list.get(j).cost + c.cost;
						int index = -1;
						...
						temp.from = c;
						temp.cost = temp.cost + c.cost;
						edges.add(new City(temp)) ;
					}
	...
	public City getmin(LinkedList<City> cities, LinkedList<City> done) {
	...
	public boolean notDone(City c , LinkedList<City> done) {
		for(City city : done)
			if(c.equals(city))
				return false ;
		return true;
	...
	public void Fun_Run(ActionEvent event) {
		...
		if(box_src.getValue()!= null && box_target.getValue()!= null) {
			...
			getPath(route) ;
			txtAreaPath.setText(route.toString().substring(1 ,route.toString().length()-1 ).replaceAll(",", " =>"));
	...
	public void reset() {
		...
		pane.getChildren().remove(rectPath);
		lines = new ArrayList<>();
		for(int i =0 ; i < buttons.size(); ++i)
	...
	public void getPath(LinkedList<City> route) {
